...
const Statistics = ({ladders}) => {
    const tasksCompleted = ladders.reduce((total, next) => (
        total += next.assignments.filter(a => a.completed).length
    ), 0);
    ...
    const getNextDueDate = (ladders) => {
        ...
        for (let ladder of ladders) {
            allTasks = allTasks.concat(ladder.assignments.filter(a => !a.completed && (dayjs(a.dueDate) > dayjs())))
        }
